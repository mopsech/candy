...
do
    ...
    do
        ...
        do
            ...
            function savePosition(p48, p49)
                task.spawn(function()
                    pcall(function()
                        if not isfolder("CandyZone/configs") then
                            makefolder("CandyZone/configs")
                        end

                        local t24 = {}

                        if isfile("CandyZone/configs/positions.json") then
                            local ok, result = pcall(function()
                                local hs = t13.hs
                                local t25 = { readfile("CandyZone/configs/positions.json") }
                        ...
                        writefile("CandyZone/configs/positions.json", t13.hs:JSONEncode(t24))
                    end)
                end)
            end
            function loadPosition(p50, p51)
                local ok, result = pcall(function()
                    if not isfile("CandyZone/configs/positions.json") then
                        return nil
                    end

                    local hs = t13.hs
                    local t26 = { readfile("CandyZone/configs/positions.json") }
            ...
            function saveLockConfig()
                pcall(function()
                    ...
                    writefile("CandyZone/configs/locks.json", t13.hs:JSONEncode(t37))
                end)
            end
            function loadLockConfig()
                pcall(function()
                    if not isfile("CandyZone/configs/locks.json") then
                        return
                    end

                    local data = t13.hs:JSONDecode(readfile("CandyZone/configs/locks.json"))
        ...
        function savePredictionConfig()
            task.spawn(function()
                ...
                writefile("CandyZone/configs/prediction.json", t13.hs:JSONEncode(t41))
            end)
        end
        function loadPredictionConfig(p94, p95, p96, p97)
            if not isfile("CandyZone/configs/prediction.json") then
                return
            end

            local ok, result = pcall(function()
                local hs = t13.hs
                local t42 = { readfile("CandyZone/configs/prediction.json") }
        ...
        t1[31] = function()
            ...
            t13.SaveManager:SetFolder("CandyZone/configs")
        ...
        function sendAutoFarmWebhook(p112, p113, p114)
            ...
            pcall(function()
                ...
                local t55 = {
					text = "CandyZone • https://CandyZone.filho.wtf"
				}
                local v1396 = os.date("!%Y-%m-%dT%H:%M:%SZ")
                local t56 = {
					title = v1376,
					url = "https://CandyZone.filho.wtf",
					description = v1382,
					color = v1383,
					author = t49,
					thumbnail = t50,
        ...
        t1[33] = t13
        t1[34] = "version"
        t1[36] = fetch("https://CandyZone.filho.wtf/version.lua", "v0.0.5")
        t1[33][t1[34]] = t1[36]
        t1[36] = function()
            local v956 = t13
            local lib = t13.lib
    ...
    do
        ...
        t1[38] = "Name"
        t1[36][t1[38]] = "CandyZone_Floating"
        t1[36] = t13.floatingGui
        t1[38] = "ResetOnSpawn"
        t1[36][t1[38]] = false
        t1[36] = t13.floatingGui
...
local ok, result = pcall(function()
    ...
    t13.ThemeManager:SetFolder("CandyZone")
